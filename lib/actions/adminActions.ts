...
export async function getAuditLogsAction(
  ...
  try {
    ...
    const transformedLogs: AuditLogEntry[] = auditLogs.map(
      (log: {
        id: any;
        userId: any;
        action: any;
...
/**
 * Bulk invite users - delegates to invitation system
 */
export async function inviteUsersAction(
  ...
  try {
    ...
    // Import the actual invitation action to avoid duplication
    const { createBulkInvitations } = await import("./invitationActions");
