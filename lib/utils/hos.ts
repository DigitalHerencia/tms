import type { HosEntry, HosLog, HosViolation } from '@/types/compliance';
...
export function calculateHosStatus(driverId: string, hosLogs: HosLog[]): DriverHOSStatus {
  ...
  const restartAvailable =
    !lastOnDutyEnd || now.getTime() - lastOnDutyEnd.getTime() >= 34 * 60 * 60 * 1000;
  ...
  for (const log of hosLogs) {
    for (const entry of log.logs) {
      if (entry.status === 'driving') {
        const start = new Date(entry.startTime);
        const end = new Date(entry.endTime);
        totalDrivingHours += (end.getTime() - start.getTime()) / (1000 * 60 * 60);
      }
    }
  }
