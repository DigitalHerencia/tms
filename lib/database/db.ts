...
const setupDatabaseMonitoring = () => {
  if (process.env.DATABASE_LOGGING === "true") {
    prisma.$use(
      async (params: { model: string; action: any; args: any }, next: (arg0: any) => any) => {
...
export class DatabaseQueries {
  ...
  static async upsertOrganizationMembership({
    ...
    try {
      ...
      if (!organization) throw new Error(`Organization not found for clerkId: ${organizationId}`);
   ...
   */ static async getOrganizationByClerkId({ clerkId }: { clerkId: string }) {
    try {
  ...
  static async upsertOrganization(data: {
    ...
    try {
      ...
      console.error(`Error in upsertOrganization for id: ${data.clerkId}`, error);
      handleDatabaseError(error);
    }
  }
  ...
  static async deleteOrganization(clerkId: string) {
    try {
      ...
      if (!organization) {
        console.warn(`[DB] Organization with id ${clerkId} does not exist, skipping delete.`);
        return {
          success: true,
