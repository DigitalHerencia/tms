 ...
 */
async function _getOrganizationKPIs(organizationId: string): Promise<OrganizationKPIs> {
  ...
  try {
    ...
    const activeLoads = loads.filter((load: { status: string }) =>
      ["assigned", "in_transit"].includes(load.status),
    );
    const completedLoads = loads.filter((load: { status: string }) => load.status === "delivered");
    const inTransitLoads = loads.filter((load: { status: string }) => load.status === "in_transit");
    const pendingLoads = loads.filter((load: { status: string }) => load.status === "pending");
    ...
    const upcomingMaintenance = maintenanceVehicles.filter(
      (v: { nextInspectionDue: string | number | Date }) =>
        v.nextInspectionDue &&
        new Date(v.nextInspectionDue) <= new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000),
    ).length;
    ...
    const maintenanceThisWeek = maintenanceVehicles.filter(
      (v: { nextInspectionDue: string | number | Date }) =>
        v.nextInspectionDue &&
        new Date(v.nextInspectionDue) <= new Date(today.getTime() + 7 * 24 * 60 * 60 * 1000),
    ).length;

    // Calculate pending load breakdowns
    const urgentLoads = pendingLoads.filter((load: { createdAt: string | number | Date }) => {
      if (!load.createdAt) return false;
      const hoursSinceCreation =
        (today.getTime() - new Date(load.createdAt).getTime()) / (1000 * 60 * 60);
...
export const getOrganizationKPIs = unstable_cache(_getOrganizationKPIs, ["organization-kpis"], {
  revalidate: 120,
  tags: ["kpi"],
});
