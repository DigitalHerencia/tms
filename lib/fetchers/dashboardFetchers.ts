...
export const getDashboardMetrics = unstable_cache(
  async (orgId: string, userId: string): Promise<DashboardMetrics> => {
    ...
    const [
      ...
      db.load.count({
        where: {
          ...
          status: {
            in: ['assigned', 'in_transit', 'at_pickup', 'at_delivery'],
          },
        },
      }),
    ...
    const complianceScore =
      inspections > 0 ? Math.round(((inspections - overdueInspections) / inspections) * 100) : 100;
...
export const getDashboardData = async (orgId: string): Promise<DashboardData> => {
  ...
  return {
    ...
    alerts: alerts.map((alert) => ({
      ...
      entityType: alert.userId ? ('driver' as const) : ('vehicle' as const),
