    ...
    const financialMetrics = {
      ...
      averageLoadValue:
        revenueData.length > 0
          ? revenueData.reduce((sum, load) => sum + (Number(load.rate) || 0), 0) /
            revenueData.length
          : 0,
    };
  ...
  try {
    ...
    const vehicleUtilization = vehicleData.map((vehicle) => {
      ...
      const activeDays = new Set(
        completedLoads.map((load) => load.actualDeliveryDate?.toDateString()).filter(Boolean),
      ).size;
 ...
 */
export async function getFilterPresets(organizationId: string): Promise<FilterPreset[]> {
  const { userId } = await auth();
  if (!userId) {
    throw new Error('Unauthorized');
  }
...
export async function getAdvancedAnalytics(
  ...
  try {
    ...
    const comparisonMetrics: ComparisonMetrics | null = null;
