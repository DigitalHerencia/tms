...
export async function getPerformanceAnalytics(
  ...
  try {
    ...
    const totalMiles = loadsData.reduce(
      (sum: number, load: { actualMiles: any }) => sum + (Number(load.actualMiles) || 0),
      0,
    );
    const totalRevenue = loadsData.reduce(
      (sum: number, load: { rate: any }) => sum + (Number(load.rate) || 0),
      0,
    );

    // Calculate on-time delivery rate
    const onTimeDeliveries = loadsData.filter(
      (load: {
        actualDeliveryDate: string | number | Date;
  ...
  try {
    ...
    const financialMetrics = {
      ...
      totalExpenses: expenseData.reduce(
        (sum: number, expense: { amount: any }) => sum + (Number(expense.amount) || 0),
        0,
      ),
      averageLoadValue:
...
export async function getDriverAnalytics(
  ...
  try {
    ...
    const driverPerformance = driverData.map(
      (driver: { loads: any; id: any; firstName: any; lastName: any }) => {
        const loads = driver.loads;
        const completedLoads = loads.filter(
          (load: { status: string }) => load.status === "delivered",
        );
        ...
        // Calculate on-time delivery rate
        const onTimeDeliveries = completedLoads.filter(
          (load: {
            actualDeliveryDate: string | number | Date;
            scheduledDeliveryDate: string | number | Date;
          }) => {
        ...
        const onTimeRate =
          completedLoads.length > 0 ? (onTimeDeliveries.length / completedLoads.length) * 100 : 0;
...
export async function getVehicleAnalytics(
  ...
  try {
    ...
    const vehicleUtilization = vehicleData.map(
      (vehicle: { loads: any; id: any; unitNumber: any; make: any; model: any }) => {
        ...
        const activeDays = new Set(
          completedLoads
            .map((load: { actualDeliveryDate: { toDateString: () => any } }) =>
              load.actualDeliveryDate?.toDateString(),
            )
            .filter(Boolean),
        ).size;
...
async function _getDashboardSummary(
  ...
  const completedLoads = loads.filter((l: { status: string }) => l.status === "delivered").length;
  const activeLoads = loads.filter(
    (l: { status: string }) => l.status !== "delivered" && l.status !== "cancelled",
  ).length;
  const totalRevenue = loads.reduce(
    (sum: number, l: { rate: any }) => sum + Number(l.rate || 0),
    0,
  );
  const totalMiles = loads.reduce(
    (sum: number, l: { actualMiles: any }) => sum + Number(l.actualMiles || 0),
    0,
  );
  ...
  const onTimeDeliveryRate = completedLoads > 0 ? (onTimeDeliveries / completedLoads) * 100 : 0;

  const fuelConsumed = Number(fuelStats._sum.gallons || 0);
  const fuelEfficiency = fuelConsumed > 0 ? totalMiles / fuelConsumed : 0;
  const driverUtilization = activeDrivers > 0 ? (activeLoads / activeDrivers) * 100 : 0;
 ...
 */
export async function processAnalyticsData(data: any[], groupBy: string): Promise<any[]> {
...
function calculateProjections(data: any[], endDate: Date) {
  ...
  const sortedData = data.sort(
    (a, b) => new Date(a.actualDeliveryDate).getTime() - new Date(b.actualDeliveryDate).getTime(),
  );
  ...
  return {
    ...
    trend: {
      direction:
        recentData.length >= 2 && recentData[recentData.length - 1].revenue > recentData[0].revenue
          ? "upward"
          : "downward",
...
export async function getGeographicAnalytics(
  ...
  try {
    ...
    loads.forEach(
      (load: { originState: any; rate: any; actualMiles: any; destinationState: any }) => {
        // Origin state data
    ...
    const geographicData = {
      ...
      summary: {
        ...
        averageRevenuePerState:
          Array.from(stateData.values()).reduce((sum, state) => sum + state.revenue, 0) /
          stateData.size,
      },
    };
