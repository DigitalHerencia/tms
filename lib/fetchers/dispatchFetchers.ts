...
export async function getLoadStatistics(
  ...
  try {
    ...
    const [totalLoads, deliveredLoads, inTransitLoads, cancelledLoads, totalRevenue, totalMiles] =
      await Promise.all([
        // Total loads count
        db.load.count({ where }),
    ...
    const statistics = {
      ...
      averageRevenuePerMile: totalMiles._sum.actualMiles
        ? Number(totalRevenue._sum.rate || 0) / Number(totalMiles._sum.actualMiles)
        : 0,
    };
...
export async function getCustomerStatistics(orgId: string) {
  try {
    ...
    const repeatCustomers = customerStats.filter(
      (customer: { _count: { id: number } }) => customer._count.id > 1,
    );

    // Calculate additional metrics
    const totalCustomers = customerStats.length;
    const totalLoads = customerStats.reduce(
      (sum: any, customer: { _count: { id: any } }) => sum + customer._count.id,
      0,
    );
    const totalRevenue = customerStats.reduce(
      (sum: number, customer: { _sum: { rate: any } }) => sum + Number(customer._sum.rate || 0),
      0,
    );
    ...
    const statistics = {
      ...
      topCustomers: customerStats.map(
        (customer: {
          customerName: any;
          _count: { id: any };
          _sum: { rate: any; actualMiles: any };
          _avg: { rate: any };
        }) => ({
...
export async function getLoadAlerts(orgId: string, severity?: string[]) {
  try {
    ...
    unassignedLoads.forEach(
      (load: {
        id: any;
        loadNumber: any;
        customerName: any;
        scheduledPickupDate: { toLocaleDateString: () => any };
      }) => {
        alerts.push({
          ...
          details: `Customer: ${load.customerName ?? "Unknown"}, Pickup: ${
    ...
    atRiskLoads.forEach(
      (load: {
        ...
        loadNumber: any;
        customerName: any;
        scheduledDeliveryDate: { toLocaleDateString: () => any };
        driver: { firstName: any; lastName: any };
      }) => {
        alerts.push({
          id: `at-risk-${load.id}`,
...
export async function getRecentDispatchActivity(orgId: string, limit = 10) {
  try {
    ...
    const logs = await db.auditLog.findMany({
      where: {
        organizationId: orgId,
        OR: [{ entityType: "Load" }, { entityType: "Driver" }, { entityType: "dispatch" }],
      },
      include: {
        user: {
          select: {
...
export const getDispatchBoardData = unstable_cache(
  async (orgId: string, filters?: LoadFilterInput) => {
    ...
    const [loadsResult, driversResult, vehiclesResult, statsResult] = await Promise.all([
