...
async function _getComplianceDashboard(organizationId: string): Promise<any> {
  ...
  try {
    ...
    const driversInCompliance = driverCompliance.filter(
      (driver: { complianceDocuments: any[] }) => {
        const requiredDocs = ["license", "medical_certificate", "drug_test"];
        return requiredDocs.every((docType) => {
          const doc = driver.complianceDocuments.find((d: { type: string }) => d.type === docType);
          return (
            doc &&
            doc.status === "approved" &&
    ...
    const vehiclesInCompliance = vehicleCompliance.filter(
      (vehicle: { complianceDocuments: any[] }) => {
        const requiredDocs = ["registration", "insurance", "inspection"];
        return requiredDocs.every((docType) => {
          const doc = vehicle.complianceDocuments.find((d: { type: string }) => d.type === docType);
          return (
            doc &&
            doc.status === "approved" &&
            (!doc.expirationDate || new Date(doc.expirationDate) > today)
...
export async function getDriverHOSStatus(driverId: string) {
  try {
    ...
    const hosLogs: HosLog[] = recentLogs.map(
      (doc: { metadata: any; id: any; createdAt: any; status: any }) => {
...
export async function getHOSViolations(
  ...
  try {
    ...
    const transformedViolations = violations.map(
      (doc: {
        id: any;
        notes: any;
        createdAt: any;
        driver: { id: any; firstName: any; lastName: any };
      }) => ({
        id: doc.id,
        type: "other" as const,
