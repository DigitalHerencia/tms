...
class SecureSessionCache {
   ...
   */
  set(sessionId: string, userContext: UserContext, ttl = this.DEFAULT_TTL): void {
  ...
  private cleanup(): void {
    ...
    if (process.env.NODE_ENV === 'development' && cleanupCount > 0) {
      console.log(`[Session Cache] Cleaned up ${cleanupCount} expired entries`);
    }
  }
...
export const SecurityMonitoring = {
  sanitizeSensitiveData(data: Record<string, any>): Record<string, any> {
    ...
    for (const [key, value] of Object.entries(data)) {
      if (/password|token|secret|key/i.test(key)) {
        ...
        sanitized[key] = SecurityMonitoring.sanitizeSensitiveData(value as Record<string, any>);
      } else {
        sanitized[key] = value;
      }
    }
  ...
  logSecurityEvent: async (event: {
    type: 'SESSION_CREATED' | 'SESSION_INVALIDATED' | 'ACCESS_DENIED' | 'PERMISSION_ESCALATION';
    userId: string;
    orgId?: string;
