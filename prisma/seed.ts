...
async function main() {
  ...
  for (let i = 3; i <= 25; i++) {
    ...
    const existingDriver = await prisma.driver.findUnique({
      where: { id: 'the-id-you-are-trying-to-insert' },
    });
    if (existingDriver) {
    ...
    for (let i = 2; i <= 50; i++) {
      const driverId = faker.helpers.arrayElement([
        driverIvan.id,
        driverJane.id,
        ...additionalDrivers,
      ]);
      ...
      const load = await prisma.load.create({
        data: {
          ...
          originLng: Number(
            faker.location.longitude({ min: -106, max: -103, precision: 0.000001 }),
          ),
          ...
          destinationLat: Number(
            faker.location.latitude({ min: 33, max: 35, precision: 0.000001 }),
          ),
          destinationLng: Number(
            faker.location.longitude({ min: -112, max: -110, precision: 0.000001 }),
          ),
    ...
    await prisma.iftaTaxCalculation.createMany({
      data: [
        {
          id: 'calc1',
          organizationId: organization.id,
    ...
    await prisma.iftaPDFGenerationLog.createMany({
      data: [
        {
          id: 'pdfgen1',
          organizationId: organization.id,
    ...
    await prisma.iftaReportPDF.createMany({
      data: [
        {
          id: 'reportpdf1',
          organizationId: organization.id,
    ...
    // Fuel purchases
    await prisma.iftaFuelPurchase.createMany({
      data: [
        {
          id: 'fuel1',
          organizationId: organization.id,
    ...
    // Regulatory audits
    await prisma.regulatoryAudit.createMany({
      data: [
        {
          id: 'audit1',
          organizationId: organization.id,
    ...
    await prisma.jurisdictionTaxRate.createMany({
      data: [
        {
          id: 'tax1',
          organizationId: organization.id,
    ...
    await prisma.analyticsFilterPreset.createMany({
      data: [
        {
          id: 'preset1',
          organizationId: organization.id,
