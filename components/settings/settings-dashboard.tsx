...
const ROLE_TAB_ACCESS = {
  admin: ['organization', 'user', 'notifications', 'integrations', 'billing', 'audit'] as const,
  dispatcher: ['organization', 'user', 'notifications', 'integrations'] as const,
  compliance_officer: ['organization', 'user', 'notifications', 'audit'] as const,
  accountant: ['billing', 'user', 'notifications', 'audit'] as const,
  driver: ['user', 'notifications'] as const,
  viewer: ['user', 'notifications'] as const,
} as const;

type TabName = 'organization' | 'user' | 'notifications' | 'integrations' | 'billing' | 'audit';
...
export function SettingsDashboard({
  ...
  const accessibleTabs = useMemo(() => {
    return (ROLE_TAB_ACCESS[userRole as keyof typeof ROLE_TAB_ACCESS] || [
      'user',
      'notifications',
    ]) as readonly TabName[];
  }, [userRole]);

  const [activeTab, setActiveTab] = useState<string>(accessibleTabs[0] || 'user');
  const [isExporting, setIsExporting] = useState(false);
  const [isImporting, setIsImporting] = useState(false);

  // Set default tab to first accessible tab
  useEffect(() => {
    if (accessibleTabs.length > 0 && !accessibleTabs.includes(activeTab as TabName)) {
          ...
          <TabsContent value="billing" className="space-y-6">
            <BillingSettingsForm
              initial={{
                ...
                subscriptionPlan: initialData.billing?.subscriptionPlan || '',
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>View all changes made to organization and user settings</CardDescription>
      </CardHeader>
      <CardContent>
