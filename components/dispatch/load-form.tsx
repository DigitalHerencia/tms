...
interface LoadFormProps {
  orgId: string;                  // required for actions
  load?: any;                     // should include .id and orgId for edit
  loadId?: string;                // optional, preferred for update
...
export function LoadForm({ orgId, load, loadId, drivers, vehicles, onClose }: LoadFormProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Map referenceNumber to load_number for backend
    ...
    // Map driverId, vehicleId, trailerId to driver_id, etc
    ...
    try {
      ...
      if (load && (load.id || loadId)) {
        ...
        if (result.success) {
          ...
          toast({ title: "Update failed", description: result.error || "Could not update load.", variant: "destructive" });
        }
      } else {
        // Create new
        ...
        if (result.success) {
          ...
          toast({ title: "Creation failed", description: result.error || "Could not create load.", variant: "destructive" });
        }
      }
    } catch (error) {
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        type="datetime-local"
                        defaultValue={load?.scheduledPickupDate ? new Date(load.scheduledPickupDate).toISOString().slice(0,16) : ""}
                        required
                      />
                    </div>
                  </CardContent>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        type="datetime-local"
                        defaultValue={load?.scheduledDeliveryDate ? new Date(load.scheduledDeliveryDate).toISOString().slice(0,16) : ""}
                        required
                      />
                    </div>
                  </CardContent>
            ...
            <TabsContent value="assignment" className="mt-4 space-y-4">
              <div className="space-y-2">
                ...
                <Select name="driverId" defaultValue={load?.driver_id || load?.driver?.id || ""}>
                  <SelectTrigger id="driverId"><SelectValue placeholder="Not Assigned" /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Select name="vehicleId" defaultValue={load?.vehicle_id || load?.vehicle?.id || ""}>
                  <SelectTrigger id="vehicleId"><SelectValue placeholder="Not Assigned" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="">Not Assigned</SelectItem>
                    {tractors.map(v => (
              ...
              <div className="space-y-2">
                ...
                <Select name="trailerId" defaultValue={load?.trailer_id || load?.trailer?.id || ""}>
                  <SelectTrigger id="trailerId"><SelectValue placeholder="Not Assigned" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="">Not Assigned</SelectItem>
                    {trailers.map(t => (
