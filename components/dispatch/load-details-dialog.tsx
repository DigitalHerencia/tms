...
export function LoadDetailsDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto border border-gray-200 bg-black text-white">
        <DialogHeader>
          ...
          <TabsContent value="details" className="mt-4 space-y-4">
            <Card className="bg-black border border-gray-200">
              ...
              <CardContent className="space-y-2 text-sm">
                <p><span className="font-medium">Customer:</span> {load.customer?.name || ""}</p>
                <p><span className="font-medium">Priority:</span> {load.priority}</p>
                <p><span className="font-medium">Commodity:</span> {String(load.cargo)}</p>
                <p><span className="font-medium">Rate:</span> {String(load.rate)}</p>
                <p><span className="font-medium">Notes:</span> {String(load.notes)}</p>
              </CardContent>
            </Card>
            <Card className="bg-black border border-gray-200">
              <CardHeader>
              ...
              <CardContent className="space-y-2 text-sm">
                <p><MapPin className="mr-1 inline h-4 w-4 text-blue-500" /> <span className="font-medium">Origin:</span>  {String(load.origin)}</p>
                <p><Route className="mr-1 inline h-4 w-4 text-blue-500" /> <span className="font-medium">Destination:</span> {String(load.destination)}</p>
                <p><Calendar className="mr-1 inline h-4 w-4 text-blue-500" /> <span className="font-medium">Pickup:</span>  {String(load.pickupDate)}</p>
                <p><Calendar className="mr-1 inline h-4 w-4 text-blue-500" /> <span className="font-medium">Delivery:</span> {String(load.deliveryDate)}</p>
              </CardContent>
            </Card>
            {(load.driver || load.vehicle) && (
              <Card className="bg-black border border-gray-200">
                ...
                <CardContent className="space-y-2 text-sm">
                  {load.driver && (
                    <p><User className="mr-1 inline h-4 w-4 text-blue-500" /> <span className="font-medium">Driver:</span> {load.driver.name}</p>
                  )}
                  {load.vehicle && (
                    <p><Truck className="mr-1 inline h-4 w-4 text-blue-500" /> <span className="font-medium">Vehicle:</span> {load.vehicle.unit}</p>
                  )}
                </CardContent>
              </Card>
            )}
              ...
              <Select value={selectedVehicleId} onValueChange={setSelectedVehicleId}>
                ...
                <SelectContent>
                  ...
                  {activeTractors.map(v => (
                    <SelectItem key={v.id} value={v.id}>{v.unitNumber}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
        ...
        <DialogFooter className="mt-4 flex justify-between">
          <div className="flex gap-2">
            {load.status !== "completed" && load.status !== "cancelled" && (
              <Button variant="outline" size="sm" onClick={() => handleStatusUpdate("cancelled")} disabled={isUpdatingStatus}>
                Cancel Load
              </Button>
            )}
            {load.status === "assigned" && (
              <Button variant="outline" size="sm" onClick={() => handleStatusUpdate("in_transit")} disabled={isUpdatingStatus}>
                Mark In-Transit
              </Button>
            )}
            {load.status === "in_transit" && (
              <Button variant="outline" size="sm" onClick={() => handleStatusUpdate("completed")} disabled={isUpdatingStatus}>
                Mark Completed
              </Button>
            )}
          </div>
