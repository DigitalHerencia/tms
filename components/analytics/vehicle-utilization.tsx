    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-md border border-gray-200 bg-black p-4">
          <h3 className="mb-4 text-lg font-bold text-white">Miles by Vehicle</h3>
          <ChartContainer
            config={{
              miles: {
                label: 'Miles',
        ...
        <div className="rounded-md border border-gray-200 bg-black p-4">
          <h3 className="mb-4 text-lg font-bold text-white">Utilization by Vehicle (%)</h3>
          <ChartContainer
            config={{
              utilization: {
                label: 'Utilization',
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={vehicleData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <ChartTooltip content={<ChartTooltipContent />} />
                <Legend />
                <Bar dataKey="utilization" fill="--color-utilization" name="Utilization" />
              </BarChart>
            </ResponsiveContainer>
          </ChartContainer>
        </div>
      ...
      <div className="rounded-md border border-gray-200 bg-black p-4">
        <h3 className="mb-4 text-lg font-bold text-white">Vehicle Performance Metrics</h3>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {vehicleData.map((vehicle) => (
              <TableRow key={vehicle.id} className="border-b border-gray-200">
                ...
                <TableCell className="text-right text-white">
                  {typeof vehicle.fuelType === 'string' ? vehicle.fuelType.toLocaleString() : 'N/A'}
                </TableCell>
                <TableCell className="text-right text-white">
