...
interface UpcomingLoadsCardProps {
  upcomingLoads: any[];
}

export function UpcomingLoadsCard({ upcomingLoads }: UpcomingLoadsCardProps) {
  return (
            ...
            {upcomingLoads.map((load: any) => (
              <div key={load.id} className="border border-gray-700 rounded-lg p-3">
                ...
                <div className="text-sm text-gray-400 space-y-1">
                  <div>
                    Origin: <span className="text-white">{load.origin}</span>
                  </div>
                  <div>
                    Destination: <span className="text-white">{load.destination}</span>
                  </div>
                </div>
              </div>
            ))}
