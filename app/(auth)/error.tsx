...
export default function Error({ error, reset }: ErrorProps) {
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.error(`[Auth Error Boundary] Digest: ${error.digest ?? 'unknown'}`);
    }
  }, [error]);
      ...
      <div className="flex flex-col items-center space-y-6">
        <div className="flex flex-1 items-center">
          ...
          >
            ...
            <span className="text-6xl font-extrabold text-white dark:text-white">FleetFusion</span>
          </Link>
        </div>
        <span className="text-foreground text-2xl font-medium">Oops! Something went wrong.</span>
        <span className="text-muted-foreground max-w-md text-center text-base">
          We couldn't load this page. Please try refreshing, or contact support if the problem
          persists.
        </span>
        {error.digest && <span className="text-xs text-gray-500">Error code: {error.digest}</span>}
        <Button
          onClick={() => reset()}
